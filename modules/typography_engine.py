...
def render_quote_on_image(background_path: str, quote_text: str, mood: str, output_path: str = "generated/final_quote_image.png"):
    ...
    # --- 1. Render Quote Text ---
    font_size = int(style["base_size"] * (0.90 + randint(-5, 5) / 100))  # More consistent sizing
    ...
    try:
        ...
        brand_font_size = max(24, int(height * 0.035)) # Larger, more readable brand text
        brand_font = _load_font("sans", brand_font_size)
        text_x = padding
        text_y = height - padding - brand_font_size - 5
        draw.text((text_x + 2, text_y + 2), BRAND_TEXT, font=brand_font, fill=(0, 0, 0, 200))
        draw.text((text_x, text_y), BRAND_TEXT, font=brand_font, fill=(230, 230, 230, 255))
